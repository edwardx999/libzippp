 ...
 */
	...
	private:
		...
		std::unique_ptr<zip_file_t,zip_file_closer> handle_;
	...
	public:
		 ...
		 */
		inline file(zip_file_t* file) noexcept
			: handle_(file)
		{}
		 ...
		 */
		inline auto read(void* data,uint64_t length) noexcept
		{
		...
		/*
			Closes the file
			Unlike the destructor, throws errors if this fails
		*/
		void close()
		{
			auto const res=zip_fclose(handle_.get());
			if(res)
			{
				throw zip_exception("Failed to close file");
			}
			handle_.reset();
		}
	};
	...
	namespace detail {
		...
		struct get_source_buffer {
			template<typename Buffer>
			static source get(Buffer data)
			{
				using VT=typename Buffer::value_type;
				static_assert(std::is_trivially_copyable<VT>::value,"Need binary data type");
				return[data=std::move(data)](struct zip* archive) -> struct zip_source* {
					auto size=data.size()*sizeof(VT);
					auto ptr=static_cast<VT*>(std::malloc(size));
			...
			static source get(Char* data)
			{
				auto container=std::basic_string<std::decay_t<Char>>(data);
				return get_source_buffer<decltype(container)>::get(std::move(container));
			}
		};
	}
	/**
	 * Add a file to the archive using a binary buffer or a c-string.
	 *
	 * \param data the buffer container or c-string
	 * \return the source to add
	 */
	template<typename Buffer>
	source source_buffer(Buffer&& data) noexcept
	{
		return detail::get_source_buffer::get(std::forward<Buffer>(data));
	}
	...
	private:
		...
		std::unique_ptr<struct zip,zip_archive_closer> handle_;
	...
	private:
		static void check_comment_length(size_t size)
		{
			constexpr size_t max_val_st=std::numeric_limits<zip_uint16_t>::max();
			if(size>max_val_st)
			{
				throw std::invalid_argument("Comment size too large");
			}
		}
	public:
		...
		*/
		...
		{
			return zip_strerror(handle_.get());
		}

		/**
			Discards the archive;
		*/
		void discard()
		{
			zip_discard(handle_.get());
			handle_.reset();
		}

		/**
			Closes the archive;
			Unlike the destructor throw errors if the close fails.
		*/
		void close()
		{
			auto const res=zip_close(handle_.get());
			if(res<-1)
			{
				throw zip_exception(error());
			}
			handle_.reset();
		}
		 ...
		 * \throw std::runtime_error on errors
		 */
		void set_file_comment(uint64_t index,std::string_view text={},flags_t flags=0)
		{
			auto size=text.size();
			check_comment_length(size);
			auto cstr=(size==0)?nullptr:text.data();
		 ...
		 * \throw std::runtime_error on errors
		 */
		std::string_view file_comment_raw(uint64_t index,flags_t flags=0) const
		{
		...
		{
			return std::string(file_comment_raw(index,flags));
		}
		 ...
		 * \throw std::runtime_error on errors
		 */
		void set_comment(std::string_view text={})
		{
			check_comment_length(text.size());
			if(zip_set_archive_comment(handle_.get(),text.data(),text.size())<0)
				throw std::runtime_error(zip_strerror(handle_.get()));
		}

		/**
		 * Get the raw archive comment.
		 *
		 * \param flags the optional flags
		 * \return the comment
		 * \throw std::runtime_error on errors
		 */
		std::string_view comment_raw(flags_t flags=0) const
		{
			int length=0;
		...
		{
			return std::string(comment_raw(flags));
		}
		 ...
		 * \throw std::runtime_error on errors
		 */
		zip_uint64_t find(const std::string& name,flags_t flags=0) const
		{
		 ...
		 * \see source::buffer
		 */
		zip_uint64_t add(const source& source,const std::string& name,flags_t flags=0)
		{
		 ...
		 * \throw std::runtime_error on errors
		 */
		zip_uint64_t mkdir(const std::string& directory,flags_t flags=0)
		{
		 ...
		 * \throw std::runtime_error on errors
		 */
		void replace(const source& source,zip_uint64_t index,flags_t flags=0)
		{
			auto src=source(handle_.get());
			...
			{
				zip_source_free(src);
				throw zip_exception(zip_strerror(handle_.get()));
			}
		}
